...
| `CHESHIRE_PDS_EXR_COMPRESSION=zips` | PrepareDenseScene's EXR compression; the default is now `zip` (16 scanlines per block, same pixels, cheaper to write and to read in every later node) where upstream writes `zips`; `none`, `rle`, `piz` and the lossy `dwaa`/`dwab`/`b44`/`pxr24` are accepted for experiments |
