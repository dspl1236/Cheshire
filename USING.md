...
| `CHESHIRE_GPS_PAIRING_UNION=1` | with it: keep the method's pairs between GPS photos as well |
...
| `CHESHIRE_GPU_VIS_VOTES=0` | Meshing's visibility votes on the host (bucketed over all threads). Since 0.3.5 the default is the device: the decisions, the contributions folded in pixel order, and the voted vertices returned as a bitmap, so the host only appends cameras to the lists; identical to the host votes (checked under `CHESHIRE_GPU_VIS_CHECK=1`, docs/04 6t and step 10) |
| `CHESHIRE_DENSE_SFM=0` | Meshing: build the dense point cloud's SfMData with upstream's single-threaded function; the default builds the same landmarks on every core (13.8 s to 3.3 s at 884 views, docs/04 6u) |
| `CHESHIRE_DENSE_SFM_CHECK=1` | with it: also build upstream's and print "dense point cloud check: identical to upstream on all N landmarks" |
| `CHESHIRE_EXR_DEFLATE=0` / `=1` | the direct EXR reader's ZIP/ZIPS chunks inflated by OpenEXR or by libdeflate; the default is libdeflate with OpenEXR before 3.2 (the Linux bundle's 3.1: about 5 % off an 884-view Meshing on house-pc) and OpenEXR's own after (docs/04 6v) |
| `CHESHIRE_EXR_DEFLATE_CHECK=1` | with libdeflate: also read every such file through OpenEXR's readPixels, keep OpenEXR's pixels on a difference, and print "EXR deflate check: N of M files identical" at exit |
...
| `CHESHIRE_GPU_FILTER_CHECK=1` | DepthMapFilter: run upstream's CPU vote pass again for every camera and compare its modal counts with the GPU's pixel by pixel; prints "depth map filter check: N of M cameras identical to the CPU vote pass" at exit (not `CHESHIRE_FILTER_CHECK`, which is Meshing's filterByPixSize check) |
