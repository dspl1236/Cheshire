...
| `CHESHIRE_BA_PERSIST=0` | bundle adjustment builds its Ceres problem from scratch every solve, as upstream does; the default keeps one Problem across solves and applies the delta (0.3.4; sets with rigs, survey points, constraints, rotation priors, temporal smoothness or depth observations use the rebuild anyway) |
