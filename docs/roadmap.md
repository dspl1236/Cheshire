...
- **CheshireJPG for the reads** (L). **In, off by default, 2026-09-25** (step 6r, `CHESHIRE_GPU_JPEG=1`): exact on four cards and both backends, but no wall-clock gain on any box (docs/04), so it stays opt-in; FeatureExtraction's read only if a measurement puts the decode on that node's critical path. The GPU JPEG codec exists (docs/19): a baseline decoder and
  ...
  A stream per Codec with pinned async copies (docs/19), checked on the host through a deferred
  runtime with negative controls, fixed that: measured on the RX 9070 it scales to about 150
...
- **Exact mode for compare_depthmaps.py** (S). **Done 2026-09-26** (c472070: per-view decoded-pixel difference counts, `--mode exact`, bridge_matrix uses it). It prints 0.0000 for anything under 5e-5 and still
