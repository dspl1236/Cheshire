...
the HIP build matches it with identical validity masks and a median relative depth error of 0
(97.5-98.9 % of pixels within 1 %), with RDNA1 and RDNA2 bit-identical to each other. The stages that upstream runs on the CPU on every vendor (matcher, depth-map filter,
meshing votes, max-flow, visibility, texturing) are written once in CUDA dialect and build for both;
all of them except the matcher carry an in-process self-check against upstream's CPU path that
counts differences (0 on an 884-view set, docs/04). No SYCL and no LLVM build needed on the user's side: the HIP SDK
on Windows and ROCm on Linux are the only prerequisites, and only for building.

**Bundling on Windows.** One package for every AMD card is 184 MB, because only five DLLs carry
...
graph (`invalid map<K, T> key`, alicevision/Meshroom#2344). Reproduced three of three on an 884-view public set,
