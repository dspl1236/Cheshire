...
Binaries are on the [v0.3.4 release](https://github.com/dspl1236/Cheshire/releases/tag/v0.3.4);
...
All four packages in v0.3.4 passed the full gates on their own hardware (docs/04), the mini6
matrix and the 41-view set. Most of this release is CPU work around GPU kernels that were already
fast: DepthMap and Texturing no longer wait on image loads, bundle adjustment's Jacobians are
analytic, and Meshing's visibility queries are built on the device. Incremental SfM can be made reproducible, and CheshireEXR, a GPU EXR decoder, is in and off
by default. What changed, in full: [docs/releases/0.3.4.md](docs/releases/0.3.4.md).
