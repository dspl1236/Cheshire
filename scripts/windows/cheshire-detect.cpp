// Cheshire: pick the runtime family and GPU target for the bundled Windows package.
//
...
// question.
//
// Prints "<family> <target>" (e.g. "rocm7.2 gfx12-generic") and exits 0, or exits 1 with a reason
// on stderr. -v also prints what each family answered.
//
...
int wmain(int argc, wchar_t** argv) {
    ...
    for (const auto& fam : kFamilies) {
        ...
        // payload from garbage, which is the exact failure this probe exists to prevent.
        if (arch.rfind("gfx", 0) != 0) {
            fwprintf(stderr, L"[detect] %ls: arch name \"%ls\" is not a gfx target - the runtime's "
                             L"device-property layout no longer matches this probe; refusing to guess\n",
    ...
    fwprintf(stderr, L"[detect] no AMD GPU with a matching payload in %ls\n", gpu.c_str());
    return 1;
}
